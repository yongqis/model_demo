...
def build_gallery(sess, input_shape, input, output, base_image_dir, gallery_data_dir):
    """
    将gallery图片进行特征编码并保存相关数据
    :param sess: a tf.Session() 用来启动模型
    :param input_shape: 图片resize的目标大小，和模型的placeholder保持一致
    :param input: 模型的输入节点，placeholder，用来传入图片
    :param output: 模型的输出节点，得到最终结果
    ...
    if not os.path.isdir(gallery_data_dir):
...
def single_query(sess, top_k, input_shape, input, output, base_image_dir, gallery_data_dir):
    """
